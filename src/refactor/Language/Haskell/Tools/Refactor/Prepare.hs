...
parseTyped ms'' = withAlteredDynFlags (return . normalizeFlags) $ do
  hsc_env' <- getSession
  ...
  let renamedSource = changeDecls <$> (tm_renamed_source tc)
  parseTrf <- runTrf (fst annots) (getPragmaComments $ snd annots) $ trfModule ms'' (pm_parsed_source p')
  let a = trfModuleRename ms'' parseTrf (fromJust $ renamedSource) (pm_parsed_source p')
  let b = runTrf (fst annots) (getPragmaComments $ snd annots) a
  c <- addTypeInfos (typecheckedSource tc) =<< b
  liftIO $ putStrLn (srcInfoDebug c)
  withTempSession (\e -> e { hsc_dflags = ms_hspp_opts ms'' })
    $ (if hasCppExtension then prepareASTCpp else prepareAST) srcBuffer . placeComments (fst annots) (getNormalComments $ snd annots)
        <$> (pure c)
    where
      changeDecls (a,decls,b,c)= let d = filter (isGoodSrcSpan . getLoc) decls in (a,d,b,c)
